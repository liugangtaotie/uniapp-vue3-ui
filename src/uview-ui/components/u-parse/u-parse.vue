...
var { windowWidth, platform } = uni.getSystemInfoSync(),
  ...
  methods: {
    ...
    setContent(html, append) {
      ...
      var fill = (target) => {
        ...
        if (this.domain && src) {
          if (src[0] == "/") {
            if (src[1] == "/")
              target.src =
                (this.domain.includes("://") ? this.domain.split("://")[0] : "") + ":" + src;
            else target.src = this.domain + src;
          } else if (!src.includes("://") && src.indexOf("data:") != 0)
            target.src = this.domain + "/" + src;
        }
      };
      // 图片处理
      ...
      for (let i = 0, j = 0, img; (img = imgs[i]); i++) {
        ...
        if (!img.hasAttribute("ignore") && img.parentElement.nodeName != "A") {
          img.i = j++;
          _ts.imgList.push(
            img.getAttribute("original-src") || img.src || img.getAttribute("data-src")
          );
      ...
      for (var link of links) {
        link.onclick = function (e) {
          ...
          if (jump && href) {
            ...
            } else if (href.indexOf("http") == 0 || href.indexOf("//") == 0) return true;
            else
              uni.navigateTo({
                url: href,
              });
      ...
      for (let video, i = 0; (video = videos[i++]); ) {
        ...
        video.onplay = function () {
          if (_ts.autopause)
            for (let item, i = 0; (item = _ts.videoContexts[i++]); ) if (item != this) item.pause();
        };
      }
      // 音频处理
    ...
    navigateTo(obj) {
      ...
      var selector = uni
        ...
        .select(
          (this._in ? this._in.selector : "#_top") +
            (obj.id ? `${d}#${obj.id},${this._in ? this._in.selector : "#_top"}${d}.${obj.id}` : "")
        )
        .boundingClientRect();
      if (this._in)
        selector
      ...
      selector.exec((res) => {
        ...
        var scrollTop =
          res[1].scrollTop + res[0].top - (res[2] ? res[2].top : 0) + (obj.offset || 0);
