<template>
  <view class="u-field" :class="{ 'u-border-top': borderTop, 'u-border-bottom': borderBottom }">
    ...
    >
      <view class="u-label" :class="[required ? 'u-required' : '']" :style="contentStyle">
        ...
        <slot name="icon"></slot>
        <text class="u-label-text" :class="[slots.icon || icon ? 'u-label-left-gap' : '']">{{
          label
        }}</text>
      </view>
      <view class="fild-body">
        <view class="u-flex-1 u-flex" :style="inputWrapStyle">
          ...
          <input
            ...
            :value="value"
            :password="password || props.type === 'password'"
            :placeholder="placeholder"
            :placeholderStyle="placeholderStyle"
            :disabled="disabled"
            :maxlength="inputMaxlength"
...
export default defineComponent({
  ...
  emits: ["input", "focus", "blur", "confirm", "input", "right-icon-click", "click"],
  ...
  setup(props: any, { emit }) {
    ...
    const errorStyle = computed(() => {
      let style: any = {};
      style.paddingLeft = props.labelWidth + "rpx";
      return styleObjToString(style);
    });
    ...
    const contentStyle = computed(() => {
      ...
      style.flex = props.labelPosition == "left" ? `0 0 ${props.labelWidth}rpx` : "1";
      return styleObjToString(style);
    });
