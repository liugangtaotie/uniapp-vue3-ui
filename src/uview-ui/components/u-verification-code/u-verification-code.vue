  ...
  setup(props: any, { emit }) {
    ...
    function checkKeepRunning() {
      ...
      if (props.keepRunning && lastTimestamp && lastTimestamp > nowTimestamp) {
        // 剩余尚未执行完的倒计秒数
        state.secNum = lastTimestamp - nowTimestamp;
        // 清除本地保存的变量
    ...
    function start() {
      ...
      changeEvent(props.changeText.replace(/x|X/, state.secNum));
      setTimeToStorage();
    ...
    function setTimeToStorage() {
      if (!props.keepRunning || !state.timer) return;
      // 记录当前的时间戳，为了下次进入页面，如果还在倒计时内的话，继续倒计时
      // 倒计时尚未结束，结果大于0；倒计时已经开始，就会小于初始值，如果等于初始值，说明没有开始倒计时，无需处理
      if (state.secNum > 0 && state.secNum <= props.seconds) {
        // 获取当前时间戳(+ new Date()为特殊写法)，除以1000变成秒，再去除小数部分
        let nowTimestamp = Math.floor(+new Date() / 1000);
        // 将本该结束时候的时间戳保存起来 => 当前时间戳 + 剩余的秒数
        uni.setStorage({
          key: props.uniqueKey + "_$uCountDownTimestamp",
          data: nowTimestamp + Number(state.secNum),
        });
      }
    }
