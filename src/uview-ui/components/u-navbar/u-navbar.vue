...
export default defineComponent({
  ...
  setup(props: any) {
    ...
    const navbarInnerStyle = computed(() => {
      ...
      style.height = props.navbarHeight + "px";
      // // 如果是各家小程序，导航栏内部的宽度需要减少右边胶囊的宽度
      // #ifdef MP
      let rightButtonWidth = systemInfo.windowWidth - menuButtonInfo.left;
      style["margin-right"] = rightButtonWidth + "px";
      // #endif
    ...
    const navbarStyle = computed(() => {
      let style: any = {};
      style["z-index"] = props.zIndex ? props.zIndex : zIndex.navbar;
      // 合并用户传递的背景色对象
      Object.assign(style, props.background);
      return styleObjToString(style);
    });

    props.backTextStyle = styleObjToString(props.backTextStyle);
    ...
    const titleStyle = computed(() => {
      ...
      style.right =
        rightButtonWidth -
        (systemInfo.windowWidth - uni.upx2px(props.titleWidth)) / 2 +
        rightButtonWidth +
        "px";
      // #endif
    ...
    function goBack() {
      ...
      if (typeof props.customBack === "function") {
        ...
        props.customBack.bind($parent.call(instanceProxy))();
      } else {
