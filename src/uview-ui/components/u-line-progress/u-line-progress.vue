<template>
  <view class="u-progress" :style="progressSuperStyle">
    <view
      :class="[
        type ? `u-type-${type}-bg` : '',
        striped ? 'u-striped' : '',
...
export default defineComponent({
  ...
  setup(props: any) {
    const progressStyle = computed(() => {
      ...
      if (props.activeColor) style.background = props.activeColor;

      return styleObjToString(style);
    });

    const progressSuperStyle = computed(() => {
      let style: any = {};
      style["border-radius"] = props.round ? "100rpx" : 0;
