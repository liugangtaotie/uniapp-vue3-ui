...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    ...
    const uZIndex = computed(() => {
      return props.zIndex ? props.zIndex : zIndex.noNetwork;
    });
  ...
  mounted() {
    instanceProxy = this;
    instanceProxy.isIOS = uni.getSystemInfoSync().platform === "ios";
    uni.onNetworkStatusChange((res) => {
      instanceProxy.isConnected = res.isConnected;
      instanceProxy.networkType = res.networkType;
    });
    uni.getNetworkType({
      success: (res) => {
        instanceProxy.networkType = res.networkType;
        if (res.networkType == "none") {
          instanceProxy.isConnected = false;
        } else {
          instanceProxy.isConnected = true;
        }
      },
    });
  },
