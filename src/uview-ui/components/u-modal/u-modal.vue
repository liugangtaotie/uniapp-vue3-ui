<template>
  <view>
    ...
    >
      <view class="u-model">
        <view v-if="showTitle" class="u-model__title u-line-1" :style="titleStyleStr">{{
          title
        }}</view>
        <view class="u-model__content">
          <view :style="contentStyleStr" v-if="$slots.default || $slots.$default">
            <slot />
          </view>
          <view v-else class="u-model__content__message" :style="contentStyleStr">{{
            content
          }}</view>
        </view>
 ...
 * @property {String} confirm-color 确认按钮的文字内容（默认"#2abc6d"）
...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    ...
    const titleStyleStr = computed(() => {
      return styleObjToString(props.titleStyle);
    });

    const contentStyleStr = computed(() => {
      return styleObjToString(props.contentStyle);
    });
    ...
    watch(
      () => props.value,
      (newVal, oldVal) => {
        if (newVal === true) props.loading = false;
      }
    );
