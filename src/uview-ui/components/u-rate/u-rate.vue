...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    let state = reactive({
      ...
      activeIndex: props.value != -1 ? props.value : props.current,
      starWidth: 0, // 每个星星的宽度
      starWidthArr: [] as any, //每个星星最右边到组件盒子最左边的距离
    });
    ...
    const customStyle = computed(() => {
      return styleObjToString(
        Object.assign(
          {},
          { "font-size": props.size + "rpx", padding: `0 ${props.gutter / 2 + "rpx"}` }
        )
      );
    });
    ...
    const elActiveIcon = computed(() => {
      ...
      if (len && len <= props.count) {
        const step = Math.round(state.activeIndex / Math.round(props.count / len));
    ...
    function touchMove(e) {
      ...
      state.activeIndex = index > props.count ? props.count : index;
      // 对最少颗星星的限制
      if (state.activeIndex < props.minCount) state.activeIndex = props.minCount;
      emitEvent();
    }
    ...
    function emitEvent() {
      ...
      // 同时修改双向绑定的value的值
      if (props.value != -1) {
        emit("input", state.activeIndex);
      }
    }

    function showDecimalIcon(index) {
      return props.disabled && parseInt(state.activeIndex) === index;
    }

    console.info("props.count", props.count);
