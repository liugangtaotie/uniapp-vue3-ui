...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    ...
    onMounted(() => {
      ...
      nextTick(() => {
        uni.$once("uOnReachBottom", () => {
          if (!state.isShow) state.isShow = true;
        });
      });
      ...
      setTimeout(() => {
        ...
        const contentObserver = (uni as any).createIntersectionObserver(instanceProxy);
        // 要理解这里怎么计算的，请看这个：
        // https://blog.csdn.net/qq_25324335/article/details/83687695
        contentObserver
          .relativeToViewport({
            bottom: props.getThreshold,
          })
          .observe(".u-lazy-item-" + state.elIndex, (res) => {
        ...
        props.contentObserver = contentObserver;
      }, 30);
    });
    ...
    const getThreshold = computed(() => {
      ...
      let thresholdPx = uni.upx2px(Math.abs(props.threshold));
    ...
    function clickImg() {
      ...
      if (state.isShow == false) whichImg = "lazyImg";
      // 如果isError为true，意味着图片加载失败，这是只剩下错误的占位图，所以点击的只能是错误占位图
      // 当然，也可以给错误的占位图元素绑定点击事件，看你喜欢~
      else if (state.isError == true) whichImg = "errorImg";
    ...
    function imgLoaded() {
      ...
      // 真正的图片加载完成
      else if (state.loadStatus == "lazyed") {
    ...
    function disconnectObserver(observerName) {
      const observer: any = [observerName];
