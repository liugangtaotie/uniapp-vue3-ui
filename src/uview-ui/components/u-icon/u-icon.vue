 ...
 * @example <u-icon name="photo" color="#2abc6d" size="28"></u-icon>
 */
  ...
  setup(props: any, { emit }) {
    const customClass = computed(() => {
      ...
      if (
        ...
        props.inactiveColor &&
        config.type.includes(props.inactiveColor)
      ) {
    ...
    const iconStyle = computed(() => {
      let style: any = {};
      style = {
        "font-size": props.size == "inherit" ? "inherit" : addUnit(props.size),
      ...
      } else if (props.color && !config.type.includes(props.color)) style.color = props.color;
