...
export default defineComponent({
  ...
  setup(props: any) {
    ...
    const iconName = computed(() => {
      ...
      if (
        ["error", "warning", "success", "info"].indexOf(state.tmpConfig.type) >= 0 &&
        state.tmpConfig.icon
      ) {
        let icon = type2icon(state.tmpConfig.type);
        return icon;
      } else {
        let icon = type2icon("success");
        return icon;
      }
    });
    ...
    // 显示toast组件，由父组件通过state.$refs.xxx.show(options)形式调用
    function show(options) {
      // 不降结果合并到state.config变量，避免多次条用u-toast，前后的配置造成混论
      ...
      state.timer = setTimeout(() => {
        ...
        timeEnd();
      }, state.tmpConfig.duration);
    }

    // 隐藏toast组件，由父组件通过state.$refs.xxx.hide()形式调用
    function hide() {
      state.isShow = false;
      if (state.timer) {
        // 清除定时器
