<template>
  <!-- 支付宝小程序使用$u.getRect()获取组件的根元素尺寸，所以在外面套一个"壳" -->
  <view>
    <view class="u-index-bar">
      <slot />
      <view
        v-if="showSidebar"
        class="u-index-bar__sidebar"
        @touchstart.stop.prevent="onTouchMove"
        @touchmove.stop.prevent="onTouchMove"
        @touchend.stop.prevent="onTouchStop"
        @touchcancel.stop.prevent="onTouchStop"
      >
        <view
          v-for="(item, index) in indexList"
          :key="index"
          class="u-index-bar__index"
          :style="{
            zIndex: zIndex + 1,
            color: activeAnchorIndex === index ? activeColor : '',
          }"
          :data-index="index"
        >
          {{ item }}
        </view>
      </view>
      <view
        class="u-indexed-list-alert"
        v-if="touchmove && indexList[touchmoveIndex]"
        :style="{
          zIndex: alertZIndex,
        }"
      >
        <text>{{ indexList[touchmoveIndex] }}</text>
      </view>
    </view>
  </view>
</template>

<script lang='ts'>
/**
 * indexList 索引列表
 * @description 通过折叠面板收纳内容区域,搭配<u-index-anchor>使用
 * @tutorial https://www.uviewui.com/components/indexList.html#indexanchor-props
 * @property {Number String} scroll-top 当前滚动高度，自定义组件无法获得滚动条事件，所以依赖接入方传入
 * @property {Array} index-list 索引字符列表，数组（默认A-Z）
 * @property {Number String} z-index 锚点吸顶时的层级（默认965）
 * @property {Boolean} sticky 是否开启锚点自动吸顶（默认true）
 * @property {Number String} offset-top 锚点自动吸顶时与顶部的距离（默认0）
 * @property {String} highlight-color 锚点和右边索引字符高亮颜色（默认#2abc6d）
 * @event {Function} select 选中右边索引字符时触发
 * @example <u-index-list :scrollTop="scrollTop"></u-index-list>
 */
import { defineComponent, computed, reactive, toRefs, watch } from "vue";
import { zIndex } from "../../libs/config/zIndex";

let instanceProxy: any;

var indexList = function () {
  var indexList: any = [];
  var charCodeOfA = "A".charCodeAt(0);
  for (var i = 0; i < 26; i++) {
    indexList.push(String.fromCharCode(charCodeOfA + i));
  }
  return indexList;
};

export default defineComponent({
  name: "u-index-list",

  props: {
    sticky: {
      type: Boolean,
      default: true,
    },
    zIndex: {
      type: [Number, String],
      default: "",
    },
    scrollTop: {
      type: [Number, String],
      default: 0,
    },
    offsetTop: {
      type: [Number, String],
      default: 0,
    },
    indexList: {
      type: Array,
      default() {
        return indexList();
      },
    },
    activeColor: {
      type: String,
      default: "#2abc6d",
    },
  },

  emits: ["select"],

  setup(props: any, { emit }) {
    let state = reactive({
      activeAnchorIndex: 0,
      sidebar: "" as any,
      showSidebar: true,
      children: [] as any,
      touchmove: false as any,
      touchmoveIndex: 0,
    });

    // #ifdef H5
    props.stickyOffsetTop = props.offsetTop ? uni.upx2px(props.offsetTop) : 44;
    // #endif
    // #ifndef H5
    props.stickyOffsetTop = props.offsetTop ? uni.upx2px(props.offsetTop) : 0;
    // #endif
    // 只能在created生命周期定义children，如果在data定义，会因为循环引用而报错
    // props.children = [];

    watch(
      () => props.scrollTop,
      (newVal, oldVal) => {
        updateData();
      }
    );

    // 弹出toast的z-index值
    const alertZIndex = computed(() => {
      return zIndex.toast;
    });

    function updateData() {
      props.timer && clearTimeout(props.timer);
      props.timer = setTimeout(() => {
        state.showSidebar = !!state.children.length;
        setRect().then(() => {
          onScroll();
        });
      }, 0);
    }

    function setRect() {
      return Promise.all([setAnchorsRect(), setListRect(), setSiderbarRect()]);
    }

    function setAnchorsRect() {
      return Promise.all(
        state.children.map((anchor: any, index) =>
          anchor.$uGetRect(".u-index-anchor-wrapper").then((rect) => {
            Object.assign(anchor, {
              height: rect.height,
              top: rect.top,
            });
          })
        )
      );
    }

    function setListRect() {
      return (
        instanceProxy &&
        instanceProxy.$uGetRect(".u-index-bar").then((rect) => {
          Object.assign(instanceProxy, {
            height: rect.height,
            top: rect.top + props.scrollTop,
          });
        })
      );
    }

    function setSiderbarRect() {
      return (
        instanceProxy &&
        instanceProxy.$uGetRect(".u-index-bar__sidebar").then((rect) => {
          state.sidebar = {
            height: rect.height,
            top: rect.top,
          };
        })
      );
    }

    function getActiveAnchorIndex() {
      const { children } = state;
      const { sticky } = props;
      for (let i = children.length - 1; i >= 0; i--) {
        const preAnchorHeight = i > 0 ? children[i - 1].height : 0;
        const reachTop = sticky ? preAnchorHeight : 0;
        if (reachTop >= children[i].top) {
          return i;
        }
      }
      return -1;
    }

    function onScroll() {
      const { children = [] } = state;
      console.info("children.length", children.length);
      if (!children.length) {
        return;
      }
      const { sticky, stickyOffsetTop, zIndex, scrollTop, activeColor } = props;
      const active = getActiveAnchorIndex();
      state.activeAnchorIndex = active;
      console.info("activeAnchorIndex", state.activeAnchorIndex);

      if (sticky) {
        let isActiveAnchorSticky = false;
        if (active !== -1) {
          isActiveAnchorSticky = children[active].top <= 0;
        }
        children.forEach((item, index) => {
          if (index === active) {
            let wrapperStyle: any = "";
            let anchorStyle: any = {
              color: `${activeColor}`,
            };
            if (isActiveAnchorSticky) {
              wrapperStyle = {
                height: `${children[index].height}px`,
              };
              anchorStyle = {
                position: "fixed",
                top: `${stickyOffsetTop}px`,
                zIndex: `${zIndex ? zIndex : zIndex.indexListSticky}`,
                color: `${activeColor}`,
              };
            }
            item.active = active;
            item.wrapperStyle = wrapperStyle;
            item.anchorStyle = anchorStyle;
          } else if (index === active - 1) {
            const currentAnchor = children[index];
            const currentOffsetTop = currentAnchor.top;
            const targetOffsetTop =
              index === children.length - 1 ? props.top : children[index + 1].top;
            const parentOffsetHeight = targetOffsetTop - currentOffsetTop;
            const translateY = parentOffsetHeight - currentAnchor.height;
            const anchorStyle = {
              position: "relative",
              transform: `translate3d(0, ${translateY}px, 0)`,
              zIndex: `${zIndex ? zIndex : zIndex.indexListSticky}`,
              color: `${activeColor}`,
            };
            item.active = active;
            item.anchorStyle = anchorStyle;
          } else {
            item.active = false;
            item.anchorStyle = "";
            item.wrapperStyle = "";
          }
        });
      }
    }

    function onTouchMove(event) {
      state.touchmove = true;
      const sidebarLength = state.children.length;
      const touch = event.touches[0];
      const itemHeight = state.sidebar.height / sidebarLength;
      let clientY = 0;
      clientY = touch.clientY;
      let index = Math.floor((clientY - state.sidebar.top) / itemHeight);
      if (index < 0) {
        index = 0;
      } else if (index > sidebarLength - 1) {
        index = sidebarLength - 1;
      }
      state.touchmoveIndex = index;
      scrollToAnchor(index);
    }

    function onTouchStop() {
      state.touchmove = false;
      props.scrollToAnchorIndex = null;
    }

    function scrollToAnchor(index) {
      if (props.scrollToAnchorIndex === index) {
        return;
      }
      props.scrollToAnchorIndex = index;
      const anchor = state.children.find((item) => item.index === props.indexList[index]);
      state.activeAnchorIndex = index;
      console.info("anchorindex-----------", index);
      console.info("state.activeAnchorIndex-----------", state.activeAnchorIndex);
      console.info("indexList-----------", props.indexList);
      if (anchor) {
        emit("select", anchor.index);
        uni.pageScrollTo({
          duration: 0,
          scrollTop: anchor.top + props.scrollTop,
        });
      }
    }

    return {
      ...toRefs(state),
      ...toRefs(props),
      alertZIndex,
      updateData,
      setRect,
      setAnchorsRect,
      setListRect,
      setSiderbarRect,
      getActiveAnchorIndex,
      onScroll,
      onTouchMove,
      onTouchStop,
      scrollToAnchor,
    };
  },

  mounted() {
    instanceProxy = this;
  },
});
</script>

<style lang="scss" scoped>
@import "../../libs/css/style.components.scss";

.u-index-bar {
  position: relative;
}

.u-index-bar__sidebar {
  position: fixed;
  top: 50%;
  right: 0;
  z-index: 99;
  display: flex;
  flex-direction: column;
  text-align: center;
  user-select: none;
  transform: translateY(-50%);
}

.u-index-bar__index {
  padding: 8rpx 18rpx;
  font-size: 22rpx;
  font-weight: 500;
  line-height: 1;
}

.u-indexed-list-alert {
  position: fixed;
  top: 50%;
  right: 90rpx;
  z-index: 9999999;
  align-items: center;
  justify-content: center;
  width: 120rpx;
  height: 120rpx;
  padding: 0;
  margin-top: -60rpx;
  font-size: 50rpx;
  color: #fff;
  background-color: rgba(0, 0, 0, 0.65);
  border-radius: 24rpx;
  @include vue-flex;
}

.u-indexed-list-alert text {
  line-height: 50rpx;
}
</style>
