...
import { defineComponent, inject, computed, watch, reactive, toRefs } from "vue";
...
export default defineComponent({
  ...
  setup(props: any) {
    let uForm: any = inject("uForm") || null;
    let state = reactive({
      initialValue: "", // 存储的默认值
    ...
    //   () => uForm.errorType,
    //   (newVal, oldVal) => {
    //     // state.errorType = newVal;
    //     broadcastInputError();
    //   }
    // );

    // label的宽度
    const elLabelWidth: any = computed(() => {
    ...
    // label的对齐方式
    const elLabelAlign: any = computed(() => {
      return props.labelAlign
        ? props.labelAlign
        : state.parentData.labelAlign
        ? state.parentData.labelAlign
        : "left";
    });
    ...
    const elLabelStyle = computed(() => {
      const styleObj = Object.assign(
        Object.keys(props.labelStyle).length
          ? props.labelStyle
          : state.parentData.labelStyle
          ? state.parentData.labelStyle
          : {},
        {
          "justify-content":
    ...
    // label的位置，左侧或者上方
    const elLabelPosition: any = computed(() => {
      return props.labelPosition
        ? props.labelPosition
        : state.parentData.labelPosition
        ? state.parentData.labelPosition
        : "left";
    });
    ...
    function broadcastInputError() {
      ...
      instanceProxy.broadcast(
        "u-input",
        "on-form-item-error",
        state.validateState === "error" && showError("border")
      );
    }
    ...
    function setRules() {
      ...
      // blur事件
      instanceProxy.$on("on-form-blur", instanceProxy.onFieldBlur);
      // change事件
      instanceProxy.$on("on-form-change", instanceProxy.onFieldChange);
    }

    // 从u-form的rules属性中，取出当前u-form-item的校验规则
    function getRules() {
      // 父组件的所有规则
      let rules = instanceProxy.parent.rules;
      rules = rules ? rules[props] : [];
      // 保证返回的是一个数组形式
      return [].concat(rules || []);
    }
    ...
    // 校验数据
    function validation(trigger, callback = (str) => {}) {
      // 检验之间，先获取需要校验的值
      state.fieldValue = instanceProxy.parent.model[props];
    ...
    // 清空当前的u-form-item
    function resetField() {
      instanceProxy.parent.model[props] = state.initialValue;
  ...
  mounted() {
    ...
    // instanceProxy.parent = instanceProxy.$parent.call(this, "u-form");
    // if (this.parent) {
    ...
    //   if (this.prop) {
    //     // 将本实例添加到父组件中
    //     instanceProxy.parent.fields.push(this);
    //     this.errorType = (this.parent as any).errorType;
    //     // 设置初始值
  ...
  beforeUnmount() {
    ...
    if (this.parent && this.prop) {
      instanceProxy.parent.fields.map((item, index) => {
        if (item === this) instanceProxy.parent.fields.splice(index, 1);
      });
    }
  },
});
