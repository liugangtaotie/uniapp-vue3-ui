...
export default defineComponent({
  ...
  emits: ["input", "blur", "focus", "confirm", "click"],
  ...
  setup(props: any, { emit }) {
    ...
    watch(
      ...
      (newVal, oldVal) => {
        ...
        if (newVal != oldVal && props.type == "select")
          handleInput({
            detail: {
              value: newVal,
            },
    ...
    const getStyle = computed(() => {
      ...
      style.minHeight = props.height
        ? props.height + "rpx"
        : props.type == "textarea"
        ? state.textareaHeight + "rpx"
        : state.inputHeight + "rpx";
      style = Object.assign(style, props.customStyle);
      return style;
    });

    const getCursorSpacing = computed(() => {
      return Number(props.cursorSpacing);
    });

    // 光标起始位置
    const uSelectionStart = computed(() => {
      return String(props.selectionStart);
    });

    // 光标结束位置
    const uSelectionEnd = computed(() => {
      return String(props.selectionEnd);
    });
    ...
    function handleInput(event) {
      ...
      // 过一个生命周期再发送事件给u-form-item，否则emit('input')更新了父组件的值，但是微信小程序上
      // 尚未更新到u-form-item，导致获取的值为空，从而校验混论
      // 这里不能延时时间太短，或者使用this.$nextTick，否则在头条上，会造成混乱
      setTimeout(() => {
        // 头条小程序由于自身bug，导致中文下，每按下一个键(尚未完成输入)，都会触发一次@input，导致错误，这里进行判断处理
        // #ifdef MP-TOUTIAO
        if (trim(value) == state.lastValue) return;
    ...
    function handleBlur(event) {
      ...
      setTimeout(() => {
        // 头条小程序由于自身bug，导致中文下，每按下一个键(尚未完成输入)，都会触发一次@input，导致错误，这里进行判断处理
        // #ifdef MP-TOUTIAO
        if (trim(event.detail.value) == state.lastValue) return;
        state.lastValue = event.detail.value;
        // #endif
        // 将当前的值发送到 u-form-item 进行校验
