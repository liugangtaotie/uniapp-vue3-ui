...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    let state = reactive({
      ...
      timer: null as any, // 用作长按的定时器
      changeFromInner: false, // 值发生变化，是来自内部还是外部
      innerChangeTimer: null as any, // 内部定时器
    });

    state.inputVal = Number(props.value);
    ...
    watch(
      ...
      (newVal: any, oldVal) => {
        ...
        let tmp = test.number(newVal);
        if (tmp && newVal >= props.min && newVal <= props.max) value = newVal;
        else value = oldVal;
        // 判断是否只能输入大于等于0的整数
