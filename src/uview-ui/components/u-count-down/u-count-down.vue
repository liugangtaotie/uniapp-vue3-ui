<template>
  <view class="u-countdown">
    ...
    <view class="u-countdown-item" :style="itemStyle" v-if="showHours">
      <view class="u-countdown-time" :style="fontStyle">
        {{ h }}
      </view>
    </view>
    <view class="u-countdown-colon" :style="colorStyle" v-if="showHours">
      {{ separator == "colon" ? ":" : "时" }}
    </view>
    ...
    <view class="u-countdown-colon" :style="colorStyle" v-if="showMinutes">
      {{ separator == "colon" ? ":" : "分" }}
    </view>
    ...
    <view class="u-countdown-colon" :style="colorStyle" v-if="showSeconds && separator == 'zh'">
      秒
    </view>
  </view>
</template>
...
import { defineComponent, reactive, toRefs, computed, onBeforeUnmount, watch } from "vue";
...
export default defineComponent({
  ...
  setup(props: any, { emit }) {
    ...
    const letterStyle = computed(() => {
      let style: any = {};
      if (props.fontSize) style["font-size"] = props.fontSize + "rpx";
    ...
    const colorStyle = computed(() => {
      let style: any = {};
      style.fontSiz = props.separatorSize + "rpx";
      style.color = props.separatorColor;
      style.paddingBottom = props.separator == "colon" ? "4rpx" : 0;
      return styleObjToString(style);
    });
  ...
  mounted() {
    instanceProxy = this;
    // 如果自动倒计时
    instanceProxy.autoplay && instanceProxy.timestamp && instanceProxy.start();
  },
});
</script>
