...
export default defineComponent({
  ...
  setup() {
    let state = reactive({
      ...
      rules: {
        name: [
          ...
          // 		state.$u.post('/ebapi/public_api/index').then(res => {
        ...
        intro: [
          ...
          // 正则校验示例，此处用正则校验是否中文，此处仅为示例，因为uView有state.$u.test.chinese可以判断是否中文
          {
            pattern: /^[\u4e00-\u9fa5]+$/gi,
            message: "简介只能为中文",
            trigger: "change",
        ...
        rePassword: [
          ...
          {
            validator: (rule, value, callback) => {
              return value === state.model.password;
            },
            message: "两次输入的密码不相等",
            trigger: ["change", "blur"],
          },
    ...
    const borderCurrent = computed(() => {
      // instanceProxy.$refs.uForm.setRules(state.rules);
    });

    function submit() {
      instanceProxy.$refs.uForm.validate((valid) => {
        if (valid) {
          if (!state.model.agreement) return toast("请勾选协议");
          console.log("验证通过");
        } else {
          console.log("验证失败");
        }
    ...
    function actionSheetCallback(index) {
      uni.hideKeyboard();
      state.model.sex = state.actionSheetList[index].text;
    }

    // checkbox选择发生变化
    function checkboxGroupChange(e) {
      state.model.likeFruit = e;
    }

    // radio选择发生变化
    function radioGroupChange(e) {
      state.model.payType = e;
    }

    // 勾选版权协议
    function checkboxChange(e) {
      state.model.agreement = e.value;
    }

    // 选择地区回调
    function regionConfirm(e) {
      state.model.region = e.province.label + "-" + e.city.label + "-" + e.area.label;
    }

    // 选择商品类型回调
    function selectConfirm(e) {
      state.model.goodsType = "";
      e.map((val, index) => {
        state.model.goodsType += state.model.goodsType == "" ? val.label : "-" + val.label;
      });
    }
    ...
    function radioCheckboxChange(index) {
      if (index == 0) {
        state.radioCheckWrap = false;
        state.radioCheckWidth = "auto";
      } else if (index == 1) {
        state.radioCheckWrap = true;
        state.radioCheckWidth = "auto";
      } else if (index == 2) {
        state.radioCheckWrap = false;
        state.radioCheckWidth = "50%";
      }
    }

    function labelPositionChange(index) {
      state.labelPosition = index == 0 ? "left" : "top";
    }
    ...
    function errorChange(index) {
      if (index == 0) state.errorType = ["message"];
      if (index == 1) state.errorType = ["toast"];
      if (index == 2) state.errorType = ["border-bottom"];
      if (index == 3) state.errorType = ["border"];
    }
