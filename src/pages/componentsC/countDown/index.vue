...
export default defineComponent({
  ...
  setup() {
    ...
    function timestampChange(index) {
      state.timestamp = index == 0 ? 60 : index == 1 ? 86400 : 983272;
    }

    function separatorChange(index) {
      state.separator = index == 0 ? "colon" : "zh";
    }

    function styleChange(index) {
      if (index == 0) {
        state.showBorder = true;
        state.borderColor = color["primary"];
        state.color = color["primary"];
        state.separatorColor = color["primary"];
      } else {
        state.showBorder = false;
        state.borderColor = "#303133";
        state.color = "#303133";
        state.separatorColor = "#303133";
      }
    }

    function showDaysChange(index) {
      state.showDays = index == 0 ? true : false;
    }

    function fontSizeChange(index) {
      state.fontSize = index == 0 ? 26 : index == 1 ? 30 : 34;
    }
