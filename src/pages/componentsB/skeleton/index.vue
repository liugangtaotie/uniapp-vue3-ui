...
export default defineComponent({
  ...
  setup(props: any) {
    ...
    const current = computed(() => {
      return state.loading ? 0 : 1;
    });

    // 点击骨架动画
    function animationChange(index) {
      state.animation = index == 0 ? true : false;
      getData();
    }
    ...
    function styleChange(index) {
      if (index == 0) {
        state.elColor = color["primary"];
        state.borderRadius = 14;
      } else {
        state.elColor = "#e5e5e5";
        state.borderRadius = 10;
      }
      getData();
    }
