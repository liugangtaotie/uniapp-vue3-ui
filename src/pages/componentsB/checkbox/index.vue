...
export default defineComponent({
  ...
  setup() {
    ...
    function sizeChange(index) {
      state.size = index == 0 ? 30 : index == 1 ? 40 : 50;
    }

    function defaultChooseChange(index) {
      // 特别处理对第一个选的选中的情况，涉及到提示语，选中状态等
      // 实际开发中不会存在这些情况，只是演示用
      if (index == 0) {
        state.list[0].checked = true;
        state.result = [state.list[0].name];
      } else {
        state.list[0].checked = false;
        state.result.splice(state.result.indexOf(state.list[0].name), 1);
      }
    }

    function maxChange(index) {
      state.max = index + 1;
    }

    function disabledChange(index) {
      if (index == 0) {
        state.list[0].disabled = true;
      } else {
        state.list[0].disabled = false;
      }
    }

    function activeColorChange(index) {
      // 如果用户尚未勾选任何checkbox，切换颜色时，默认选中第一个让用户看到效果，因为勾选了才有效果
      if (!state.result.length) state.list[0].checked = true;
      let theme =
        index == 0
          ? "primary"
          : index == 1
          ...
          ? "success"
          : "info";
      state.activeColor = color[theme];
    }

    // 选中某个复选框时，由checkbox时触发
    function checkboxChange(e) {
      state.result.push(e.name);
    }
    ...
    function widthChange(index) {
      state.width = index == 0 ? "50%" : "";
    }

    function wrapChange(index) {
      state.wrap = !index;
    }
